...
def check_password():
    ...
    # 🤫 ബാക്ക്ഗ്രൗണ്ടിൽ ഒരു 10 അക്ക വലിയ രഹസ്യ നമ്പർ ഉണ്ടാക്കുന്നു
    if "secret_serial" not in st.session_state:
        st.session_state.secret_serial = "".join([str(random.randint(0, 9)) for _ in range(10)])
    ...
    with col_p2:
        ...
        if user_phone.strip():
            # 🕵️‍♂️ വാട്സാപ്പിൽ വലിയൊരു സിസ്റ്റം ഐഡി പോലെ ഈ നമ്പർ കാണിക്കുന്നു
            msg = f"Hello Pradeep, I want to login to Astro App.\n📞 Phone: {user_phone.strip()}\n⚙️ System Ref ID: {st.session_state.secret_serial}"
            encoded_msg = urllib.parse.quote(msg)
            whatsapp_url = f"https://wa.me/{MY_WHATSAPP_NUMBER}?text={encoded_msg}"
            
            st.link_button("📲 Get Login Code via WhatsApp", whatsapp_url, use_container_width=True)
        
        st.write("---")
        st.info("💡 മുകളിലെ ബട്ടൺ ക്ലിക്ക് ചെയ്ത് പ്രദീപിന്റെ വാട്സാപ്പിലേക്ക് മെസ്സേജ് അയക്കുക. അവിടെ നിന്ന് ലഭിക്കുന്ന ഒടിപി താഴെ ടൈപ്പ് ചെയ്യുക.")
        
        # 🔑 യഥാർത്ഥ ഒടിപി എന്ന് പറയുന്നത് ആ 10 അക്ക നമ്പറിന്റെ ആദ്യത്തെ 4 അക്കങ്ങൾ മാത്രമാണ്!
        REAL_OTP = st.session_state.secret_serial[:4]
        
        input_code = st.text_input("നിങ്ങൾക്ക് വാട്സാപ്പിൽ ലഭിച്ച കോഡ് ഇവിടെ അടിക്കുക:", type="password")
        
        if st.button("Verify & Login", use_container_width=True):
            if input_code and input_code == REAL_OTP:
            ...
            if st.button("Refresh Code / പുതിയ കോഡ് വേണോ?", type="secondary", use_container_width=True):
                st.session_state.secret_serial = "".join([str(random.randint(0, 9)) for _ in range(10)])
                st.rerun()
